...
figure('Position',[0 0  1680 933])
    ...
    % The data here is just each ROIs own ID number
        ...
        if i < 6
            ...
            cmap = lines(34);
            faceColor = 'flat'; 
            savename = ['ROIS_',boundary_type{i},'_',faceColor,'.png'];
        else
            data = lh_sulc;
            cmap = parula(100);
            if i == 6
            ...
            savename = ['sulc_',boundary_type{i},'_',faceColor,'.png'];
        end


    [p,b] = plotSurfaceROIBoundary(surface,lh_rand200,data,boundary_type{i},cmap,linewidth, 'FaceColor', faceColor);
    ...
    p.Clipping = 'off';
    if strcmp(boundary_type, 'faces'); b.boundary.Clipping = 'off';
    else; for bi = b.boundary; bi.Clipping = 'off'; end;                end
    p.EdgeColor = 'k';
    p.EdgeAlpha = .5;
