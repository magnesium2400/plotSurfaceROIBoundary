...
warning('Using plotSurfaceROIBoundary is not recommended. Use patchp or plotBrain instead.'); 
...
addParameter(ip, 'FaceColor', 'interp', @(x) any(strcmp({'flat', 'interp'}, x)) );
...
if strcmp(boundary_method, 'faces')
    ...
    boundaryOptions = {'BoundaryOptions', {'LineWidth', linewidth}}; 
end

[p, boundary_plot, BOUNDARY] = patchp('Vertices', verts, 'Faces', faces, 'Parcellation', rois, 'FaceVertexCData', data, ...
    'FaceColor', face_color_method, 'PatchOptions', patch_options, ...
    'BoundaryMethod', boundary_method, 'BoundaryColor', boundary_color, ...
    boundaryOptions{:});

material dull;
colormap(gca, cmap());
if ~isempty(climits); try caxis(climits); catch; end; end %#ok<CAXIS>
